...
export const SCENE_1_DURATION = 65;
...
export const Scene1Hook: React.FC = () => {
  ...
  const kenBurns = interpolate(frame, [0, SCENE_1_DURATION], [1, 1.14], {
    easing: Easing.out(Easing.cubic),
  });

  // The shot opens in broad daylight (the real photo, untouched) and grades
  // down into dusk/night fast as the tension builds — day-for-night grade.
  const grade = interpolate(frame, [3, 16], [0, 1], {
  ...
  const vignette = interpolate(frame, [3, 16], [0, 0.78], {
    easing: Easing.out(Easing.cubic),
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const danger = interpolate(frame, [36, SCENE_1_DURATION], [0, 1], {
    easing: Easing.in(Easing.cubic),
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  const { opacity: overlayOpacity, translateY: overlayY } = enterUp(frame, 19, 9, 16);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: colors.navyDeepest }}>
      ...
      <Stage gap={22} justify="flex-end" padTop={90} padBottom={150}>
        <Kicker delay={22}>Central Vigía</Kicker>
        <Heading delay={28} size={72}>
          ¿Y si alguien entra
          <br />
